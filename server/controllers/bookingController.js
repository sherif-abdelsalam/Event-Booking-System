...
exports.cancelBooking = catchAsync(async (req, res, next) => {
  ...
  if (booking.userId.toString() !== req.user.id && req.user.role !== "admin") {
    return next(new AppErrors(`Not authorized to delete this booking`, 403));
  }
  ...
  if (event && new Date() > cancelData) {
    return next(
      new AppErrors(`Cannot cancel a booking for an event at this time`, 400)
    );
  }

  // Delete the booking instead of just changing status
...
exports.getMyBookings = catchAsync(async (req, res, next) => {
  const bookings = await Booking.find({
    ...
    .populate({
      path: "eventId",
      select: "name description date venue price imageUrl category",
      populate: {
        path: "category",
        select: "name",
      },
